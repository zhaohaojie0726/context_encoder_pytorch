...
parser.add_argument('--cuda', action='store_true', help='disables cuda')
...
if opt.dataset in ['imagenet', 'folder', 'lfw']:
    ...
    dataset = dset.ImageFolder(root=opt.dataroot,
                               transform=transforms.Compose([
                                   transforms.Resize(opt.imageSize),  # 修改为 Resize
    ...
    transform = transforms.Compose([transforms.Resize(opt.imageSize),  # 修改为 Resize
                                    transforms.CenterCrop(opt.imageSize),
                                    transforms.ToTensor(),
                                    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
...
for epoch in range(resume_epoch, opt.niter):
    for i, data in enumerate(dataloader, 0):
        ...
        input_cropped.data[:, 0, int(opt.imageSize / 4 + opt.overlapPred):int(
        ...
        input_cropped.data[:, 1, int(opt.imageSize / 4 + opt.overlapPred):int(
            opt.imageSize / 4 + opt.imageSize / 2 - opt.overlapPred),
        int(opt.imageSize / 4 + opt.overlapPred):int(
            opt.imageSize / 4 + opt.imageSize / 2 - opt.overlapPred)] = 2 * 104.0 / 255.0 - 1.0
        input_cropped.data[:, 2, int(opt.imageSize / 4 + opt.overlapPred):int(
            opt.imageSize / 4 + opt.imageSize / 2 - opt.overlapPred),
        int(opt.imageSize / 4 + opt.overlapPred):int(
        ...
        # train with real
        netD.zero_grad()
        label.data.resize_(batch_size, 1).fill_(real_label)  # 修改这里，保证尺寸一致
